...
name <- paste("continental_cluster_", simulation1, "-", simulation2, "-", simulation3, ".png", sep="")
png(name)
par(mfrow=c(3,1))
barplot(t(as.matrix(individuals1)), col=rainbow(6), names.arg=lab1, ylab="Ancestry", main = paste("Sampling", simulation1, "(K=6)"), border=NA)
barplot(t(as.matrix(individuals2)), col=rainbow(6), names.arg=lab2, ylab="Ancestry", main = paste("Sampling", simulation2, "(K=6)"), border=NA)
barplot(t(as.matrix(individuals3)), col=rainbow(6), names.arg=lab3, ylab="Ancestry", main = paste("Sampling", simulation3, "(K=6)"), border=NA)
dev.off()
...
chemin <- "C:/Users/Loïc/Documents/UNIL/Master 2.1/eHGDP/eHGDP/eHGDP_individualsWO.txt"
data <- read.table(chemin, h=F)
#ind <- data[data != 6]
